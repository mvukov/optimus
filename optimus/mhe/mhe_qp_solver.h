...
#ifndef OPTIMUS_MHE_MHE_QP_SOLVER_H_
#define OPTIMUS_MHE_MHE_QP_SOLVER_H_
...
bool MheQpSolver<nx, nu, nc>::Solve() {
  ...
  if (llt_q.info() != Eigen::Success) {
    OPTIMUS_PRINT("Failed to factorize q at init!");
    return false;
  }
  ...
  llt_q.compute(p_inv);
  if (llt_q.info() != Eigen::Success) {
    OPTIMUS_PRINT("Failed to factorize Pinv at init!");
    return false;
  }
  ...
  for (size_t node = 1; node < num_intervals; ++node) {
    ...
    llt_q.compute(big_sigma);
    if (llt_q.info() != Eigen::Success) {
      OPTIMUS_PRINT("Failed to factorize q at node: " << node);
      return false;
    }
    ...
    llt_q.compute(p_inv);
    if (llt_q.info() != Eigen::Success) {
      OPTIMUS_PRINT("Failed to factorize Pinv at node: " << node);
      return false;
    }
  ...
  if constexpr (nc > 0) {
    ...
    if (lapack_info) {
      OPTIMUS_PRINT("Failed to factorize Pinv for the terminal constraint!");
      return false;
    }
  }
...
#endif  // OPTIMUS_MHE_MHE_QP_SOLVER_H_
