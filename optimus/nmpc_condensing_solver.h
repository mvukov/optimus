...
#ifndef OPTIMUS_NMPC_CONDENSING_SOLVER_H_
#define OPTIMUS_NMPC_CONDENSING_SOLVER_H_
...
bool NmpcCondensingSolver<nx, nu>::Config::Validate() const {
  if (num_intervals == 0U) {
    DEBUG_OPTIMUS("The number of intervals must be >= 0!");
    return false;
  }
  if (num_working_set_recalculations == 0U) {
    DEBUG_OPTIMUS("The number of working set recalculations must be >= 0!");
    return false;
  }

  if (state_bound_indices.size() > nx ||
      !std::is_sorted(state_bound_indices.begin(), state_bound_indices.end())) {
    DEBUG_OPTIMUS("The state bound indices must be sorted!");
    return false;
  }
  if (std::adjacent_find(state_bound_indices.begin(),
                         state_bound_indices.end()) !=
      state_bound_indices.end()) {
    DEBUG_OPTIMUS("The state bound indices must be unique!");
    return false;
  }
  for (const auto& index : state_bound_indices) {
    if (index >= nx) {
      DEBUG_OPTIMUS("All state bound indices must be >= 0 and < " << nx << "!");
      return false;
    }
  }
  ...
  if (!integrator_callback || !stage_cost_evaluator_callback ||
    ...
    DEBUG_OPTIMUS("All NMPC solver callbacks must be defined!");
    return false;
  }
...
#endif  // OPTIMUS_NMPC_CONDENSING_SOLVER_H_
